"""methodology-wide backend projection pass for A4V3 artifacts.

This pass is intentionally sidecar-only: it reads existing seed methodology ``*.a4v3`` files
...
expanded authoring frontend. methodology files use the repository-local A4V3 parser and
...
``a4v3_parser_v1`` ASTs and links to the SMT probe report instead of forcing seed methodology
IR through the incompatible frontend.

CLI:
    python IR/src/backend_projection_pass_v1.py --run-root case_studies/financial_methodology
"""
...
def _discover_targets(run_root: Path, out_root: Path) -> list[ProjectionTarget]:
    ...
    for group_dir_name, target_kind in ENTRY_GROUPS:
        ...
        for entry_dir in sorted(group_dir.iterdir(), key=lambda p: p.name.lower()):
            ...
            if not main_ir.exists() or main_ir.stat().st_size == 0:
                continue
            rel = main_ir.relative_to(run_root).as_posix()
            targets.append(
                ProjectionTarget(
    ...
    supplemental = [
        ("bridge_main_bridge", "bridge", run_root / "bridge" / "main_bridge.a4v3"),
        ("bridge_resolved_bridge_decisions_v1", "bridge", run_root / "bridge" / "resolved_bridge_decisions_v1.a4v3"),
        ("merge_canonical_ontology_v1", "merge", run_root / "merge" / "canonical_ontology_v1.a4v3"),
        ("process_ontology_v1", "process", run_root / "process" / "process_ontology_v1.a4v3"),
        ("ordinary_rebalance_workflow_v1", "process", run_root / "process" / "ordinary_rebalance_workflow_v1.a4v3"),
        ("exception_overlays_v1", "process", run_root / "process" / "exception_overlays_v1.a4v3"),
    ]
...
def run_pass(run_root: Path, out_root: Path) -> dict[str, Any]:
    targets = _discover_targets(run_root, out_root)
    target_reports = [_project_target(target) for target in targets]
    union_report = _write_owl_union(out_root, target_reports)
    resolved_report = _write_resolved_owl_union(run_root, out_root, union_report)
    ...
    smt_report_path = run_root / "reasoning" / "smt_probe_results_v1.json"
    smt_summary = None
    ...
    return {
        "schema": "backend_projection_report_v1",
...
def _write_owl_union(out_root: Path, target_reports: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    union_path = union_dir / "methodology_owl_union.ttl"
    manifest_path = union_dir / "methodology_owl_union_manifest.json"
    check_path = union_dir / "methodology_owl_union.check.json"
    ...
    manifest = {
        "schema": "methodology_owl_union_manifest_v1",
        "target_count": len(target_reports),
...
def _bridge_resolution_data(run_root: Path) -> dict[str, Any]:
...
def _write_resolved_review_reports(
    *,
    ...
    payload = {
        "schema": "methodology_owl_resolved_review_items_v1",
...
def _write_resolved_owl_union(run_root: Path, out_root: Path, union_report: dict[str, Any]) -> dict[str, Any]:
    union_path = Path(union_report["union_path"])
    manifest_path = Path(union_report["manifest_path"])
    union_dir = union_path.parent
    resolved_path = union_dir / "methodology_owl_resolved.ttl"
    resolved_manifest_path = union_dir / "methodology_owl_resolved_manifest.json"
    resolved_check_path = union_dir / "methodology_owl_resolved.check.json"
    review_items_path = union_dir / "methodology_owl_resolved_review_items_v1.json"
    review_summary_path = union_dir / "methodology_owl_resolved_review_items_summary.md"
    ...
    target_paths = {entry["target_id"]: run_root / entry["relative_path"] for entry in manifest.get("entries", [])}
    bridge = _bridge_resolution_data(run_root)
    ...
    if not bridge.get("available"):
        ...
        result = {
            "schema": "methodology_owl_resolved_manifest_v1",
            "status": "skipped",
...
def _write_markdown(report: dict[str, Any], out_path: Path) -> None:
    lines = ["# Financial Methodology Backend Projection Report v1", ""]
...
def main() -> None:
    ...
    parser.add_argument("--run-root", default="case_studies/financial_methodology")
    parser.add_argument("--out-root", default=None)
    args = parser.parse_args()

    run_root = Path(args.run_root)
    out_root = Path(args.out_root) if args.out_root else run_root / "backend_projection"
    report = run_pass(run_root, out_root)
