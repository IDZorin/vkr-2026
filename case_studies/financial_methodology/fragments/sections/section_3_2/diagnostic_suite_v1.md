...
- repair: Inspect relevant artifacts and apply module-specific repair rules.
- evidence:
  - `assertion_complexity.top_complex_assertions` → index_not_rebalanced_extraordinarily
...
- evidence:
  - `identifier_glue.top_glued_identifiers` → {'identifier': 'index_not_rebalanced_extraordinarily', 'raw_piece_count': 4, 'co; {'identifier': 'ExtraordinaryRebalance', 'raw_piece_count': 2, 'content_piece_co; {'identifier': 'Rebalance', 'raw_piece_count': 1, 'content_piece_count': 1, 'raw (+4 more)
  - `identifier_glue.lowest_source_grounded_identifiers` → {'identifier': 'ExtraordinaryRebalance', 'raw_piece_count': 2, 'content_piece_co; {'identifier': 'rebalance_of_index', 'raw_piece_count': 3, 'content_piece_count'; {'identifier': 'index_not_rebalanced_extraordinarily', 'raw_piece_count': 4, 'co
...
- value: **1.4285714285714286**  (from `identifier_glue.mean_identifier_piece_count_content`)
- meaning: Semantics may be packed into names; report the exact identifier, pieces, grounding ratios, and source links.
- repair: Split long identifiers into anchored functions/relations/ontology links.
- evidence:
  - `identifier_glue.top_glued_identifiers` → {'identifier': 'index_not_rebalanced_extraordinarily', 'raw_piece_count': 4, 'co; {'identifier': 'ExtraordinaryRebalance', 'raw_piece_count': 2, 'content_piece_co; {'identifier': 'Rebalance', 'raw_piece_count': 1, 'content_piece_count': 1, 'raw (+4 more)
...
- value: **4**  (from `grounding.new_formula_content_token_count_vs_text_only`)
...
- evidence:
  - `grounding.new_formula_content_tokens_vs_text_only` → extraordinaryrebalance; index_not_rebalanced_extraordinarily; rebalance_of_index (+1 more)
  - `grounding.new_full_surface_content_tokens_vs_text_only` → a4v3; canonical; computation (+21 more)
  - `lexical_coverage.formula_repeat_overuse_examples` → {'token': 'index', 'current_count': 6, 'baseline_count': 1, 'delta': 5}; {'token': 'extraordinaryrebalance', 'current_count': 2, 'baseline_count': 0, 'de; {'token': 'theindex', 'current_count': 2, 'baseline_count': 0, 'delta': 2}
  - `grounding.origin_error_count` → 0

## [WARNING] `ontology_planning` / `new_formula_token_count_vs_text_only`

- value: **5**  (from `grounding.new_formula_token_count_vs_text_only`)
- meaning: Target contains unsupported or excessive content; report the exact new/ungrounded/overused token or symbol list.
- repair: Remove unsupported content, rename symbols, or add explicit prelude/ontology/user-hint provenance.
- evidence:
  - `grounding.new_formula_content_tokens_vs_text_only` → extraordinaryrebalance; index_not_rebalanced_extraordinarily; rebalance_of_index (+1 more)
...
- value: **24**  (from `grounding.new_full_surface_content_token_count_vs_text_only`)
- meaning: Target contains unsupported or excessive content; report the exact new/ungrounded/overused token or symbol list.
- repair: Remove unsupported content, rename symbols, or add explicit prelude/ontology/user-hint provenance.
- evidence:
  - `grounding.new_formula_content_tokens_vs_text_only` → extraordinaryrebalance; index_not_rebalanced_extraordinarily; rebalance_of_index (+1 more)
