# Translation Metrics v1 - N09

- generated_at: `2026-05-20T02:55:49.180105+02:00`
- artifact_path: `case_studies\financial_methodology\definitions\N09\N09_manual_section_workspace_artifact_current_v1.json`
- catalog_version: `translation_metrics_catalog_v1`

## validity

```json
{
  "ast_valid": 1,
  "ast_error_count": 0,
  "rendering_ok": 1,
  "combined_validation_ok": 1,
  "invalid_submit_count": 0,
  "repair_orchestrator_count": 0,
  "semantic_coverage_retry_count": 0,
  "status": "ok",
  "ast_conformance": "canonical_ast_v1",
  "rendering_status": "rendered_from_ast"
}
```

## grounding

```json
{
  "ungrounded_symbol_count": 0,
  "ungrounded_sort_count": 0,
  "ungrounded_ref_count": 0,
  "ungrounded_callee_count": 0,
  "prelude_redeclaration_count": 0,
  "origin_error_count": 0,
  "new_formula_token_count_vs_text_only": 9,
  "new_formula_content_token_count_vs_text_only": 8,
  "new_formula_token_count_vs_text_prelude_only": 9,
  "new_formula_content_token_count_vs_text_prelude_only": 8,
  "new_formula_token_count_vs_text_prelude_advisory": 9,
  "new_formula_content_token_count_vs_text_prelude_advisory": 8,
  "new_full_surface_token_count_vs_text_only": 33,
  "new_full_surface_content_token_count_vs_text_only": 28,
  "new_full_surface_token_count_vs_text_prelude_only": 31,
  "new_full_surface_content_token_count_vs_text_prelude_only": 27,
  "new_full_surface_token_count_vs_text_prelude_advisory": 31,
  "new_full_surface_content_token_count_vs_text_prelude_advisory": 27,
  "new_formula_content_tokens_vs_text_only": [
    "eligible_rebalance_day",
    "eligible_rebalance_day_definition",
    "eligiblerebalanceday",
    "eurexexchange",
    "londonstockexchange",
    "newyorkstockexchange",
    "tokyostockexchange",
    "trading_day_at_exchange"
  ],
  "new_formula_content_tokens_vs_text_prelude_only": [
    "eligible_rebalance_day",
    "eligible_rebalance_day_definition",
    "eligiblerebalanceday",
    "eurexexchange",
    "londonstockexchange",
    "newyorkstockexchange",
    "tokyostockexchange",
    "trading_day_at_exchange"
  ],
  "new_formula_content_tokens_vs_text_prelude_advisory": [
    "eligible_rebalance_day",
    "eligible_rebalance_day_definition",
    "eligiblerebalanceday",
    "eurexexchange",
    "londonstockexchange",
    "newyorkstockexchange",
    "tokyostockexchange",
    "trading_day_at_exchange"
  ],
  "new_full_surface_content_tokens_vs_text_only": [
    "a4v3",
    "canonical",
    "computation",
    "current",
    "deterministic",
    "draft",
    "eligible_rebalance_day",
    "eligible_rebalance_day_definition",
    "eligiblerebalanceday",
    "eurexexchange",
    "londonstockexchange",
    "main_ir",
    "manual",
    "metric",
    "newyorkstockexchange",
    "parsed",
    "primitive_usage",
    "recomputation",
    "reconstructed",
    "rendering_notes",
    "residual_risks",
    "section",
    "section-level",
    "strengths",
    "text",
    "tokyostockexchange",
    "trading_day_at_exchange",
    "workspace"
  ],
  "new_full_surface_content_tokens_vs_text_prelude_only": [
    "canonical",
    "computation",
    "current",
    "deterministic",
    "draft",
    "eligible_rebalance_day",
    "eligible_rebalance_day_definition",
    "eligiblerebalanceday",
    "eurexexchange",
    "londonstockexchange",
    "main_ir",
    "manual",
    "metric",
    "newyorkstockexchange",
    "parsed",
    "primitive_usage",
    "recomputation",
    "reconstructed",
    "rendering_notes",
    "residual_risks",
    "section",
    "section-level",
    "strengths",
    "text",
    "tokyostockexchange",
    "trading_day_at_exchange",
    "workspace"
  ],
  "new_full_surface_content_tokens_vs_text_prelude_advisory": [
    "canonical",
    "computation",
    "current",
    "deterministic",
    "draft",
    "eligible_rebalance_day",
    "eligible_rebalance_day_definition",
    "eligiblerebalanceday",
    "eurexexchange",
    "londonstockexchange",
    "main_ir",
    "manual",
    "metric",
    "newyorkstockexchange",
    "parsed",
    "primitive_usage",
    "recomputation",
    "reconstructed",
    "rendering_notes",
    "residual_risks",
    "section",
    "section-level",
    "strengths",
    "text",
    "tokyostockexchange",
    "trading_day_at_exchange",
    "workspace"
  ]
}
```

## coverage

```json
{
  "normalized_clause_count": 1,
  "formula_bearing_item_count": 1,
  "formula_to_clause_compression_ratio": 1.0,
  "coverage_fragment_count": 0,
  "covered_formally_count": 0,
  "covered_only_in_notes_count": 0,
  "missing_fragment_count": 0,
  "coverage_fragment_formal_ratio": 1.0,
  "coverage_fragment_any_ratio": 1.0,
  "prose_leak_count": 0
}
```

## lexical_coverage

```json
{
  "reference_text_basis": "normalized_clauses_or_source_excerpt_fallback",
  "source_content_token_count": 8,
  "source_content_token_mass": 14,
  "formula_content_token_count": 10,
  "formula_content_token_mass": 34,
  "full_surface_content_token_count": 33,
  "full_surface_content_token_mass": 61,
  "formula_content_token_recall": 0.5,
  "full_surface_content_token_recall": 0.5,
  "full_surface_content_token_jaccard": 0.10810810810810811,
  "formula_content_token_multiset_recall": 0.5,
  "formula_content_token_multiset_precision": 0.20588235294117646,
  "formula_repeat_overuse_token_count": 10,
  "formula_repeat_overuse_mass": 27,
  "formula_repeat_underuse_token_count": 5,
  "formula_repeat_underuse_mass": 7,
  "formula_repeat_overuse_examples": [
    {
      "token": "exchange",
      "current_count": 11,
      "baseline_count": 4,
      "delta": 7
    },
    {
      "token": "trading",
      "current_count": 5,
      "baseline_count": 1,
      "delta": 4
    },
    {
      "token": "eligiblerebalanceday",
      "current_count": 3,
      "baseline_count": 0,
      "delta": 3
    },
    {
      "token": "eligible",
      "current_count": 3,
      "baseline_count": 1,
      "delta": 2
    },
    {
      "token": "rebalance",
      "current_count": 3,
      "baseline_count": 1,
      "delta": 2
    },
    {
      "token": "eurexexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "londonstockexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "newyorkstockexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "tokyostockexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "definition",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    }
  ],
  "full_surface_content_token_multiset_recall": 0.5,
  "full_surface_content_token_multiset_precision": 0.11475409836065574,
  "full_surface_repeat_overuse_token_count": 33,
  "full_surface_repeat_overuse_mass": 54,
  "full_surface_repeat_underuse_token_count": 5,
  "full_surface_repeat_underuse_mass": 7,
  "full_surface_repeat_overuse_examples": [
    {
      "token": "exchange",
      "current_count": 11,
      "baseline_count": 4,
      "delta": 7
    },
    {
      "token": "trading",
      "current_count": 5,
      "baseline_count": 1,
      "delta": 4
    },
    {
      "token": "eligiblerebalanceday",
      "current_count": 3,
      "baseline_count": 0,
      "delta": 3
    },
    {
      "token": "eligible",
      "current_count": 3,
      "baseline_count": 1,
      "delta": 2
    },
    {
      "token": "rebalance",
      "current_count": 3,
      "baseline_count": 1,
      "delta": 2
    },
    {
      "token": "eurexexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "londonstockexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "manual",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "metric",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "newyorkstockexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "section",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "tokyostockexchange",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "workspace",
      "current_count": 2,
      "baseline_count": 0,
      "delta": 2
    },
    {
      "token": "a4v3",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    },
    {
      "token": "canonical",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    },
    {
      "token": "computation",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    },
    {
      "token": "current",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    },
    {
      "token": "definition",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    },
    {
      "token": "deterministic",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    },
    {
      "token": "draft",
      "current_count": 1,
      "baseline_count": 0,
      "delta": 1
    }
  ],
  "source_to_formula_token_gap_count": 4,
  "source_to_full_surface_token_gap_count": 4,
  "source_content_tokens_missing_from_formula": [
    "eurex",
    "london",
    "stock",
    "tokyo"
  ],
  "source_content_tokens_missing_from_full_surface": [
    "eurex",
    "london",
    "stock",
    "tokyo"
  ]
}
```

## source_vs_normalized

```json
{
  "source_excerpt_content_token_count": 8,
  "source_excerpt_content_token_mass": 14,
  "normalized_content_token_count": 8,
  "normalized_content_token_mass": 14,
  "normalized_clause_count": 1,
  "normalized_content_token_recall_from_source": 1.0,
  "normalized_content_token_precision_to_source": 1.0,
  "normalized_content_token_jaccard": 1.0,
  "normalized_content_token_multiset_recall_from_source": 1.0,
  "normalized_content_token_multiset_precision_to_source": 1.0,
  "source_to_normalized_token_gap_count": 0,
  "normalized_to_source_new_token_count": 0,
  "normalized_repeat_overuse_token_count": 0,
  "normalized_repeat_overuse_mass": 0,
  "normalized_repeat_underuse_token_count": 0,
  "normalized_repeat_underuse_mass": 0,
  "normalized_repeat_overuse_examples": [],
  "normalized_length_ratio_vs_source_mass": 1.0,
  "normalized_content_mass_per_clause": 14.0,
  "source_normalized_bertscore_precision": 0.9999999403953552,
  "source_normalized_bertscore_recall": 0.9999999403953552,
  "source_normalized_bertscore_f1": 0.9999999403953552,
  "normalized_implies_source_entailment": 0.973739504814148,
  "source_implies_normalized_entailment": 0.973739504814148,
  "source_vs_normalized_contradiction_score": 0.014658071100711823
}
```

## semantic_preservation

```json
{
  "explicit_link_violation_count": 0,
  "scope_visibility_violation_count": 0,
  "exception_visibility_violation_count": 0,
  "counterfactual_loss_count": 0,
  "clarification_loss_count": 0,
  "responsibility_loss_count": 0
}
```

## definition_quality

```json
{
  "focus_term_explicitly_modeled": 0,
  "focus_term_in_top_level_decl": 0,
  "focus_term_in_formula_body": 0,
  "declaration_only_downgrade_flag": 0,
  "definition_body_present": 1,
  "vacuous_constraint_flag": 0,
  "reflexive_equality_count": 0
}
```

## compression

```json
{
  "notes_token_count": 29,
  "notes_content_token_count": 23,
  "notes_to_formula_content_ratio": 2.3
}
```

## identifier_glue

```json
{
  "identifier_count": 10,
  "compound_identifier_count_raw": 3,
  "compound_identifier_count_content": 6,
  "compound_identifier_rate_raw": 0.3,
  "compound_identifier_rate_content": 0.6,
  "max_identifier_piece_count_raw": 4,
  "max_identifier_piece_count_content": 4,
  "mean_identifier_piece_count_raw": 2.5,
  "mean_identifier_piece_count_content": 2.3,
  "identifier_glue_excess_mass_raw": 3,
  "identifier_glue_excess_mass_content": 8,
  "identifier_glue_excess_rate_raw": 0.3,
  "identifier_glue_excess_rate_content": 0.8,
  "source_grounded_content_piece_ratio_mean": 0.875,
  "advisory_grounded_content_piece_ratio_mean": 0.875,
  "low_source_grounded_glued_identifier_count": 0,
  "low_source_grounded_glued_identifier_rate": 0.0,
  "entity_relation_target_fusion_count": 0,
  "entity_relation_target_fusion_rate": 0.0,
  "conditional_relation_name_packing_count": 0,
  "conditional_relation_name_packing_rate": 0.0,
  "top_glued_identifiers": [
    {
      "identifier": "eligible_rebalance_day_definition",
      "raw_piece_count": 4,
      "content_piece_count": 4,
      "raw_pieces": [
        "eligible",
        "rebalance",
        "day",
        "definition"
      ],
      "content_pieces": [
        "eligible",
        "rebalance",
        "day",
        "definition"
      ],
      "glue_excess_raw": 1,
      "glue_excess_content": 2,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 0.75,
      "advisory_grounded_content_piece_ratio": 0.75
    },
    {
      "identifier": "NewYorkStockExchange",
      "raw_piece_count": 4,
      "content_piece_count": 4,
      "raw_pieces": [
        "new",
        "york",
        "stock",
        "exchange"
      ],
      "content_pieces": [
        "new",
        "york",
        "stock",
        "exchange"
      ],
      "glue_excess_raw": 1,
      "glue_excess_content": 2,
      "source_grounded_content_piece_count": 4,
      "advisory_grounded_content_piece_count": 4,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "trading_day_at_exchange",
      "raw_piece_count": 4,
      "content_piece_count": 3,
      "raw_pieces": [
        "trading",
        "day",
        "at",
        "exchange"
      ],
      "content_pieces": [
        "trading",
        "day",
        "exchange"
      ],
      "glue_excess_raw": 1,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "EligibleRebalanceDay",
      "raw_piece_count": 3,
      "content_piece_count": 3,
      "raw_pieces": [
        "eligible",
        "rebalance",
        "day"
      ],
      "content_pieces": [
        "eligible",
        "rebalance",
        "day"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "LondonStockExchange",
      "raw_piece_count": 3,
      "content_piece_count": 3,
      "raw_pieces": [
        "london",
        "stock",
        "exchange"
      ],
      "content_pieces": [
        "london",
        "stock",
        "exchange"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "TokyoStockExchange",
      "raw_piece_count": 3,
      "content_piece_count": 3,
      "raw_pieces": [
        "tokyo",
        "stock",
        "exchange"
      ],
      "content_pieces": [
        "tokyo",
        "stock",
        "exchange"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "EUREXExchange",
      "raw_piece_count": 1,
      "content_piece_count": 1,
      "raw_pieces": [
        "eurexexchange"
      ],
      "content_pieces": [
        "eurexexchange"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 0,
      "source_grounded_content_piece_count": 0,
      "advisory_grounded_content_piece_count": 0,
      "source_grounded_content_piece_ratio": 0.0,
      "advisory_grounded_content_piece_ratio": 0.0
    },
    {
      "identifier": "Day",
      "raw_piece_count": 1,
      "content_piece_count": 1,
      "raw_pieces": [
        "day"
      ],
      "content_pieces": [
        "day"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 0,
      "source_grounded_content_piece_count": 1,
      "advisory_grounded_content_piece_count": 1,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "Exchange",
      "raw_piece_count": 1,
      "content_piece_count": 1,
      "raw_pieces": [
        "exchange"
      ],
      "content_pieces": [
        "exchange"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 0,
      "source_grounded_content_piece_count": 1,
      "advisory_grounded_content_piece_count": 1,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "d",
      "raw_piece_count": 1,
      "content_piece_count": 0,
      "raw_pieces": [
        "d"
      ],
      "content_pieces": [],
      "glue_excess_raw": 0,
      "glue_excess_content": 0,
      "source_grounded_content_piece_count": 0,
      "advisory_grounded_content_piece_count": 0,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    }
  ],
  "lowest_source_grounded_identifiers": [
    {
      "identifier": "eligible_rebalance_day_definition",
      "raw_piece_count": 4,
      "content_piece_count": 4,
      "raw_pieces": [
        "eligible",
        "rebalance",
        "day",
        "definition"
      ],
      "content_pieces": [
        "eligible",
        "rebalance",
        "day",
        "definition"
      ],
      "glue_excess_raw": 1,
      "glue_excess_content": 2,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 0.75,
      "advisory_grounded_content_piece_ratio": 0.75
    },
    {
      "identifier": "NewYorkStockExchange",
      "raw_piece_count": 4,
      "content_piece_count": 4,
      "raw_pieces": [
        "new",
        "york",
        "stock",
        "exchange"
      ],
      "content_pieces": [
        "new",
        "york",
        "stock",
        "exchange"
      ],
      "glue_excess_raw": 1,
      "glue_excess_content": 2,
      "source_grounded_content_piece_count": 4,
      "advisory_grounded_content_piece_count": 4,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "EligibleRebalanceDay",
      "raw_piece_count": 3,
      "content_piece_count": 3,
      "raw_pieces": [
        "eligible",
        "rebalance",
        "day"
      ],
      "content_pieces": [
        "eligible",
        "rebalance",
        "day"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "LondonStockExchange",
      "raw_piece_count": 3,
      "content_piece_count": 3,
      "raw_pieces": [
        "london",
        "stock",
        "exchange"
      ],
      "content_pieces": [
        "london",
        "stock",
        "exchange"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "TokyoStockExchange",
      "raw_piece_count": 3,
      "content_piece_count": 3,
      "raw_pieces": [
        "tokyo",
        "stock",
        "exchange"
      ],
      "content_pieces": [
        "tokyo",
        "stock",
        "exchange"
      ],
      "glue_excess_raw": 0,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    },
    {
      "identifier": "trading_day_at_exchange",
      "raw_piece_count": 4,
      "content_piece_count": 3,
      "raw_pieces": [
        "trading",
        "day",
        "at",
        "exchange"
      ],
      "content_pieces": [
        "trading",
        "day",
        "exchange"
      ],
      "glue_excess_raw": 1,
      "glue_excess_content": 1,
      "source_grounded_content_piece_count": 3,
      "advisory_grounded_content_piece_count": 3,
      "source_grounded_content_piece_ratio": 1.0,
      "advisory_grounded_content_piece_ratio": 1.0
    }
  ],
  "entity_relation_target_fusion_examples": [],
  "conditional_relation_name_packing_examples": []
}
```

## parameterization

```json
{
  "callable_symbol_count": 2,
  "callable_symbol_with_args_count": 2,
  "top_level_parameter_slot_count": 3,
  "quantifier_parameter_slot_count": 1,
  "total_parameter_slot_mass": 4,
  "factorization_count": 2,
  "parameter_slots_per_factor": 1.5,
  "factorization_index": 0.6666666666666666,
  "focus_symbol_signature": "missing",
  "focus_symbol_arity": null
}
```

## assertion_complexity

```json
{
  "assertion_count": 1,
  "mean_assertion_node_count": 17,
  "max_assertion_node_count": 17,
  "total_assertion_node_count": 17,
  "mean_assertion_depth": 5,
  "max_assertion_depth": 5,
  "total_ite_count": 0,
  "max_ite_count_per_assertion": 0,
  "total_quantifier_count": 1,
  "total_connective_count": 2,
  "total_branching_point_count": 4,
  "max_branching_point_count_per_assertion": 4,
  "mean_call_count_per_assertion": 5,
  "single_assertion_logic_share": 1.0,
  "overcompressed_single_assertion_flag": 0,
  "top_complex_assertions": [
    {
      "name": "eligible_rebalance_day_definition",
      "assert_kind": "constraint",
      "node_count": 17,
      "depth": 5,
      "ite_count": 0,
      "quantifier_count": 1,
      "connective_count": 2,
      "branching_point_count": 4,
      "max_fanout": 4,
      "call_count": 5
    }
  ],
  "normalized_clause_count": 1,
  "node_count_per_normalized_clause": 17.0,
  "branching_point_count_per_normalized_clause": 4.0
}
```

## normalized_alignment

```json
{
  "normalized_clause_count": 1,
  "logic_block_count": 1,
  "clause_to_logic_block_ratio": 1.0,
  "logic_block_to_clause_ratio": 1.0,
  "clause_underdecomposition_mass": 0,
  "clause_overdecomposition_mass": 0,
  "focus_symbol_arity": null,
  "helper_factorization_count": 2,
  "single_assertion_logic_share": 1.0,
  "underdecomposed_logic_flag": 0
}
```

## normalized_relative

```json
{
  "new_formula_content_token_rate_vs_reference_mass": 0.5714285714285714,
  "new_full_surface_content_token_rate_vs_reference_mass": 1.9285714285714286,
  "formula_repeat_overuse_rate": 1.9285714285714286,
  "full_surface_repeat_overuse_rate": 3.857142857142857,
  "parameter_slot_mass_per_clause": 4.0,
  "parameter_slot_mass_per_reference_token": 0.2857142857142857,
  "factorization_per_clause": 2.0,
  "factorization_per_reference_token": 0.14285714285714285,
  "notes_content_token_rate_vs_reference_mass": 1.6428571428571428
}
```

## tradeoff

```json
{
  "render_bertscore_f1_to_normalized_per_parameter_slot_mass": 0.20019806921482086,
  "render_bertscore_f1_to_source_per_parameter_slot_mass": 0.20019806921482086,
  "render_nli_ir_implies_text_per_parameter_slot_mass": 0.009805855341255665,
  "render_nli_text_implies_ir_per_parameter_slot_mass": 0.22729669511318207,
  "render_bertscore_f1_to_normalized_per_formula_repeat_overuse_mass": 0.029658973217010498,
  "render_nli_text_implies_ir_per_formula_repeat_overuse_mass": 0.03367358446121216,
  "render_nli_ir_implies_text_per_formula_repeat_overuse_mass": 0.0014527193098156541,
  "render_bertscore_f1_to_normalized_per_full_surface_repeat_overuse_mass": 0.014829486608505249,
  "render_nli_text_implies_ir_per_full_surface_repeat_overuse_mass": 0.01683679223060608,
  "pairwise_structure_similarity_mean_per_parameter_slot_mass": null,
  "focus_signature_mode_share_per_parameter_slot_mass": 0.25
}
```

## variants

```json
{
  "candidate_reading_count": 0,
  "draft_variant_count": 1,
  "unique_ir_variant_count": 1,
  "critic_selected_reading_id": "MANUAL",
  "critic_selected_reading_label": "Manual section workspace current IR",
  "critic_confidence_label": "",
  "critic_confidence_score": null,
  "critic_merge_recommended": false
}
```

## variability

```json
{
  "usable_variant_count": 1,
  "unique_variant_signature_count": 1,
  "focus_signature_unique_count": 1,
  "focus_signatures": {
    "missing": 1
  },
  "artifact_signature_entropy": 0.0,
  "focus_signature_entropy": 0.0,
  "focus_signature_mode_share": 1.0,
  "pairwise_structure_similarity_mean": null,
  "pairwise_structure_distance_mean": null,
  "pairwise_token_jaccard_mean": null,
  "parameter_slot_mass_mean": 4.0,
  "parameter_slot_mass_stddev": 0.0,
  "factorization_count_mean": 2.0,
  "factorization_count_stddev": 0.0,
  "same_parameter_mass_different_structure_pair_count": 0,
  "selected_ir_is_mode": true
}
```

## efficiency

```json
{
  "observable_llm_call_lower_bound": 0,
  "qa_json_attempt_count": 0,
  "critic_json_attempt_count": 0,
  "advisor_assistant_turn_count_lower_bound": 0,
  "selected_drafter_assistant_turn_count_lower_bound": 0,
  "wall_clock_seconds": null,
  "max_call_latency_s": null
}
```

## render_back

```json
{
  "render_back_mode": "deterministic_proxy",
  "render_back_metric_status": "proxy_only_until_llm_verbalizer_runs",
  "render_back_text": "constraint eligible rebalance day definition states that for every d of type eligible rebalance day, eligible rebalance day for d if and only if (trading day at exchange for d and new york stock exchange, trading day at exchange for d and london stock exchange, trading day at exchange for d and eurexexchange, and trading day at exchange for d and tokyo stock exchange). eligible rebalance day is a type. exchange is a type. new york stock exchange is a distinguished entity. london stock exchange is a distinguished entity. eurexexchange is a distinguished entity. tokyo stock exchange is a distinguished entity. trading day at exchange holds between day and exchange. eligible rebalance day holds between eligible rebalance day.",
  "render_bertscore_precision_to_normalized": 0.7712917923927307,
  "render_bertscore_recall_to_normalized": 0.8326393365859985,
  "render_bertscore_f1_to_normalized": 0.8007922768592834,
  "render_bertscore_precision_to_source": 0.7712917923927307,
  "render_bertscore_recall_to_source": 0.8326393365859985,
  "render_bertscore_f1_to_source": 0.8007922768592834,
  "render_nli_ir_implies_text": 0.03922342136502266,
  "render_nli_text_implies_ir": 0.9091867804527283,
  "render_nli_ir_implies_source": 0.03922342136502266,
  "render_nli_source_implies_ir": 0.9091867804527283,
  "render_nli_render_to_normalized": {
    "entailment": 0.03922342136502266,
    "neutral": 0.35885417461395264,
    "contradiction": 0.6019223928451538
  },
  "render_nli_normalized_to_render": {
    "entailment": 0.9091867804527283,
    "neutral": 0.05746320262551308,
    "contradiction": 0.03335004299879074
  },
  "render_nli_render_to_source": {
    "entailment": 0.03922342136502266,
    "neutral": 0.35885417461395264,
    "contradiction": 0.6019223928451538
  },
  "render_nli_source_to_render": {
    "entailment": 0.9091867804527283,
    "neutral": 0.05746320262551308,
    "contradiction": 0.03335004299879074
  },
  "render_contradiction_score": 0.6019223928451538
}
```

## silver_reference

```json
{
  "disabled_for_manual_reference": false,
  "silver_reference_found": false,
  "silver_reference_path": "<PRIVATE_WORKSPACE>\\IR\\outputs\\runs\\silver_baseline\\definitions_full6_multivariant_critic_v1_with_gold.md",
  "silver_reference_ir": "",
  "top_level_cosine": null,
  "logic_cosine": null,
  "arity_cosine": null,
  "silver_structure_similarity": null
}
```
