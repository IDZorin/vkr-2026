# Financial Methodology Entry Checks: N28

- generated_at: `2026-05-10T20:11:53`
- with_llm: `True`
- overall_status: `ok`

## Summary

- clean_gate: `accepted`
- blocking_conditions: ``
- semantic_lint findings: `0` (strong `0`, soft `0`)
- token coverage: `6/6` (`1.0`)
- phrase coverage: `2/2` (`1.0`)
- lowering smells: `1`
- provenance_lint findings: `0` (strong `0`, soft `0`)
- provenance back-translation: status `ok`, score `0.959`, warnings `0`
- diagnostic gate: `needs_review` (fail `0`, warning `28`)
- single semantic judge: `corresponds` / `exact_equivalence`
- multi judge: `corresponds` (agreement `0.8`, unanimous `False`)
- corpus-aware multi judge: local `corresponds` / corpus `corresponds` (agreement `1.0`)

## Uncovered Tokens

- none

## Uncovered Phrases

- none

## Steps

| step | status | returncode | duration_s |
| --- | --- | ---: | ---: |
| parser_strict | ok | 0 | 2.763 |
| semantic_lint | ok | 0 | 0.146 |
| token_provenance | ok | 0 | 2.681 |
| provenance_lint | ok | 0 | 0.22 |
| provenance_backtranslation | ok | 0 | 0.198 |
| family_coverage | ok | 0 | 0.121 |
| source_phrase_coverage | ok | 0 | 0.151 |
| lowering_audit | ok | 0 | 0.109 |
| legacy_metrics | ok | 0 | 21.446 |
| diagnostic_suite_skip_llm | ok | 0 | 2.001 |
| quality_snapshot | ok | 0 | 2.473 |
| single_semantic_judge_and_render | ok | 0 | 22.273 |
| multi_judge | ok | 0 | 45.677 |
| corpus_aware_multi_judge | ok | 0 | 64.678 |
| diagnostic_suite_with_llm | ok | 0 | 13.369 |
| quality_snapshot_final | ok | 0 | 2.327 |
